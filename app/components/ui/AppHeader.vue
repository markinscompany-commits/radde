<template>
  ...
  <header class="relative flex items-center justify-between px-5 md:px-8 max-w-1400px mx-auto w-full" :class="(scrolled || mobileOpen || solid) ? 'py-2.5' : 'py-5 md:py-8'" style="transition: padding 0.3s ease">
    ...
    <div class="hidden lg:flex items-center gap-4">
      ...
      <div class="flex items-center gap-1.5">
        <a :href="contacts.whatsapp" target="_blank" rel="noopener" title="WhatsApp" class="messenger-icon">
        ...
        <!-- Telegram и Max временно скрыты по просьбе клиента (06.2026). Код оставлен. -->
        <a v-if="false" :href="contacts.telegram" target="_blank" rel="noopener" title="Telegram" class="messenger-icon">
        ...
        <a v-if="false" :href="contacts.max" target="_blank" rel="noopener" title="Max" class="messenger-icon">
          <img :src="`${base}images/icon-max.svg`" width="16" height="16" alt="Max" style="filter: brightness(0) invert(1); opacity: 0.7;" />
        </a>
        <a :href="contacts.instagram" target="_blank" rel="noopener" title="Instagram*" class="messenger-icon">
    ...
    <a :href="`tel:${contacts.phone.tel}`" class="lg:hidden mobile-action-btn" aria-label="Позвонить">
    ...
    <!-- Mobile menu button -->
    <button @click="toggleMobile" class="lg:hidden mobile-action-btn" aria-label="Меню">
  ...
  <Transition name="slide">
    <div v-if="mobileOpen" class="fixed top-14 md:top-15 left-0 right-0 bottom-0 z-60 lg:hidden bg-sand-900/95 backdrop-blur-md border-t border-white/10 px-5 pt-5 pb-8 overflow-y-auto" data-lenis-prevent>
      ...
      <!-- Phone -->
      <a :href="`tel:${contacts.phone.tel}`" class="block font-body text-4.5 font-600 text-white mb-2">
        {{ contacts.phone.display }}
      </a>
...
const mainNav: NavLink[] = [
  { href: `${base}`, label: 'Главная', section: 'hero' },
...
function smoothScroll(href: string) {
  ...
  // Внешние страницы (/, /contacts, /blog ...) — обычный переход
  if (!isAnchor) {
    const onHome = window.location.pathname === base || window.location.pathname === base.slice(0, -1)
    if ((href === base || href === base.slice(0, -1)) && onHome) {
  ...
  // Якорь — если мы не на главной, переходим на главную с якорем
  const onHome = window.location.pathname === base || window.location.pathname === base.slice(0, -1)
  if (!onHome) {
